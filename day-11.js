...
const runRound = (rows) => {
  ...
  for (let y = 0; y < rows.length; y++) {
    for (let x = 0; x < rows[0].length; x++) {
      const seat = rows[y][x];
      if (['L', '#'].includes(seat)) {
        const occupiedAdjacentSeats = getOccupiedAdjecentSeats(rows, x, y);
        if (seat === 'L' && occupiedAdjacentSeats === 0) {
          copy[y][x] = '#';
        }
...
const getOccupiedAdjecentSeats = (rows, x, y) => {
  return [
    (rows[y - 1] || '')[x],
    (rows[y + 1] || '')[x],
    rows[y][x - 1],
