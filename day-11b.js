const solvePuzzle = (input) => {
  let rows = input.split('\n').filter(Boolean).map(it => it.split(''));
  ...
  do {
    ...
    newCount = countOccupied(rows);
  } while (newCount !== previousCount);

  return countOccupied(rows);
}

const countOccupied = (rows) => {
  return rows.reduce((acc1, curr1) => {
    return acc1 + curr1.reduce((acc2, curr2) => {
...
const runRound = (rows) => {
  ...
  for (let y = 0; y < rows.length; y++) {
    for (let x = 0; x < rows[0].length; x++) {
      const seat = rows[y][x];
      if (['L', '#'].includes(seat)) {
        const occupiedVisibleSeats = getOccupiedVisibleSeats(rows, x, y);
        if (seat === 'L' && occupiedVisibleSeats === 0) {
          copy[y][x] = '#';
        }
        if (seat === '#' && occupiedVisibleSeats >= 5) {
          copy[y][x] = 'L';
        }
      }
    }
...
const getOccupiedVisibleSeats = (rows, x, y) => {
  return [
    checkDirection(rows, x, y, 0, -1),  // N
    checkDirection(rows, x, y, 1, -1),  // NE
    checkDirection(rows, x, y, 1, 0),   // E
    checkDirection(rows, x, y, 1, 1),   // SE
    checkDirection(rows, x, y, 0, 1),   // S
    checkDirection(rows, x, y, -1, 1),  // SW
    checkDirection(rows, x, y, -1, 0),  // W
    checkDirection(rows, x, y, -1, -1), // NW
  ].filter(Boolean).length;
}

const checkDirection = (rows, x, y, deltaX, deltaY) => {
  for (let i = 1;;i++) {
    let seat = (rows[y + deltaY * i] || [])[x + deltaX * i];
    if (!seat || seat === 'L') {
      return false;
    }
    if (seat === '#') {
