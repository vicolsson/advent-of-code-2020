const solvePuzzle = (input) => {
  const rows = getRowData(input);

  const count = rows.filter(row => {
    const firstOk = row.password[row.index1] === row.letter;
    const secondOk = row.password[row.index2] === row.letter;
...
const getRowData = (input) => {
  return rows = input.split('\n').filter(Boolean).map(row => {
    const parts = row.split(' ');
    const [ range, letter, password ] = parts;
    const positions = range.split('-').map(it => parseInt(it))
