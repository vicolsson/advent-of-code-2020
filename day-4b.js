const solvePuzzle = (input) => {
  ...
  const validators = {
      'byr': it => it >= 1920 && it <= 2020,
      'iyr': it => it >= 2010 && it <= 2020,
      'eyr': it => it >= 2020 && it <= 2030,
      'hgt': it => {
        if (!it) { return false; }
        if (it.endsWith('cm')) {
          const value = it.replace('cm', '');
          return value >= 150 && value <= 193;
        }
        if (it.endsWith('in')) {
            const value = it.replace('in', '');
            return value >= 59 && value <= 76;
        }
      },
      'hcl': it => /^#[a-f0-9]{6}$/g.test(it),
      'ecl': it => ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].includes(it),
      'pid': it => /^[0-9]{9}$/g.test(it)
  };

  const validCount = docs.filter((doc) => {
    return Object.entries(validators).every(([key, validator]) => validator(doc[key]))
  }).length;

  return validCount;
}

const CORRECT_ANSWER = 147;
const INPUT = `
