const solvePuzzle = (input) => {
  const rules = input.split('\n').filter(Boolean).reduce((acc, ruleString) => {
    const [ bagDescription, containsString ] = ruleString.split(' contain ')
    const contains = containsString.replace('.', '').replace(/bags/g, 'bag').split(', ').reduce((acc, curr) => {
      const amount = parseInt(curr[0]);
      const color = curr.substr(2);
    ...
    return {
      ...acc,
      [bagDescription.replace(/bags/g, 'bag')]: contains
    };
  }, {});

  const bagsThatCouldHoldAGoldOne = [...new Set(recursivelyGoThroughBags(rules, 'shiny gold bag'))];

  return bagsThatCouldHoldAGoldOne.length;
}

const recursivelyGoThroughBags = (rules, bag) => {
  const bagsThatCanHoldBag = getBagsThatCanHoldBag(rules, bag);

  return [
    ...bagsThatCanHoldBag,
    ...bagsThatCanHoldBag.reduce((acc, curr) => {
      return [
        ...acc,
        ...recursivelyGoThroughBags(rules, curr)
      ];
    }, [])
  ];
}

const getBagsThatCanHoldBag = (rules, bag) => {
  return Object.entries(rules).filter((([key, value]) => {
    return value[bag] > 0
  })).map(([key]) => key);
};

const CORRECT_ANSWER = 131;
const INPUT = `
