const solvePuzzle = (input) => {
  ...
  const bagsInGoldenBag = recursivelyGoThroughBags(rules, 'shiny gold bag');

  return bagsInGoldenBag;
}

const recursivelyGoThroughBags = (rules, bag) => {
  const bagsInBag = rules[bag];

  if (!bagsInBag) {
    return 0;
  }
  
  return Object.entries(bagsInBag).reduce((acc, [key, value]) => {
    return acc + (value * (1 + recursivelyGoThroughBags(rules, key)))
  }, 0)
}

const CORRECT_ANSWER = 11261;
const INPUT = `
