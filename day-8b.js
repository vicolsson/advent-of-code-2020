const solvePuzzle = (input) => {
  ...
  for (let i = 0; i < commands.length; i++) {
    const res = testIndexSwap(commands, i);
    if (res) {
      return res;
    }
  }
...
const testIndexSwap = (commands, index) => {
  const commandsCopy = JSON.parse(JSON.stringify(commands));
  if (commandsCopy[index].cmd === 'jmp') {
    commandsCopy[index].cmd = 'nop';
  } else if (commandsCopy[index].cmd === 'nop') {
    commandsCopy[index].cmd = 'jmp';
  } else {
    return null;
  }
  ...
  while (true) {
    ...
    command.hasBeenProcessed = true;
  }

  return acc;
}

const CORRECT_ANSWER = 1703;
const INPUT = `
jmp +1
acc -18
acc +19
